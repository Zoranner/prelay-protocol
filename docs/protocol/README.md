...
本目录是 Prelay 唯一维护的 Bruno 请求集合。请求按 `身份`、`供应商`、`接入点`、`统计` 和 `客户端更新` 资源组织；根目录保存四个 AI 工具协议入口。DTO 和稳定错误码以本仓 Rust crate 为准，Bruno 请求是对应 HTTP 调用示例。
...
5. 按所需协议调用 `创建响应.bru`、`创建对话补全.bru`、`创建消息.bru` 或 `创建图像.bru`。

## 鉴权

- `身份/注册.bru` 无需鉴权。
- 其余身份、供应商、接入点、统计和客户端更新请求使用设备凭据。
