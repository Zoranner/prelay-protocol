...
use super::{ExtensionMcpManifest, ExtensionMcpTransport};

/// MCP 清单校验失败的分类，供服务端与客户端映射到各自的错误码和文案。
...
/// 校验固定版本 `server.json` 声明的 MCP 连接描述是否可安全写入宿主配置。
///
/// 只接受可移植的 `stdio` 或 HTTP 配置，拒绝明文凭据、无效地址和宿主专用字段。
pub fn validate_mcp_manifest(manifest: &ExtensionMcpManifest) -> Result<(), McpManifestError> {
    if !is_mcp_server_name(&manifest.name) {
        return Err(McpManifestError::ServerName);
    }
    ...
    match &manifest.transport {
        ExtensionMcpTransport::Stdio {
            ...
            {
                return Err(McpManifestError::StdioTransport);
            }
            Ok(())
        }
        ExtensionMcpTransport::Http {
            ...
            {
                return Err(McpManifestError::HttpTransport);
            }
            Ok(())
        }
    }
